...
{
    /// <summary>
    /// Cinemachine ClearShot allows the user to specify a group or collection
    /// of <see cref="CinemachineVirtualCameraBase"/>s which dynamically frame an event, or subject.
    /// When the ClearShot is enabled, it will iterate through its list
    /// of child <see cref="CinemachineVirtualCameraBase"/> in order and query each one for validity.
    /// Validity is determined by checking the <see cref="CameraState.ShotQuality"/>.
    /// The highest priorty child <see cref="CinemachineVirtualCameraBase"/> with the best quality will be chosen.
    /// </summary>
    ...
    public class CinemachineClearShot : CinemachineVirtualCameraBase
    {
        ...
        [Tooltip("Default object for the camera children to look at (the aim target), if not specified in a child camera.  May be empty.")]
        ...
        [Tooltip("Default object for the camera children wants to move with (the body target), if not specified in a child camera.  May be empty.")]
        ...
        [Tooltip("If checked, camera choice will be randomized if multiple cameras are equally desirable.  Otherwise, child list order will be used.")]
